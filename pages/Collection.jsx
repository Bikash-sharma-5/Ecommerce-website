...
const Collection = () => {
  ...
  return (
    <div className='flex flex-col m-20 sm:flex-row gap-1 sm:gap-10 pt-10 border-t'>

      {/* Filter Options */}
      <div className='min-w-60 mr-20'>
      ...
      <div className='flex-1'>
        ...
        {/* Map Products */}
        <div className='grid grid-cols-2 md:grid-cols-1  place-items-center lg:grid-cols-2 gap-4 gap-y-6'>
          {
            filterProducts.map((item, index) => (
              <ProductItem2 key={index} id={item._id} image={item.image} name={item.name} price={item.price} />
            ))
          }
        </div>
      </div>
